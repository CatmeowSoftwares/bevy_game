...
fn update_walled(
    ...
    for (entity, ground_detection, global_transform, mut movement) in &mut query {
        ...
        let left_hit = spatial_query.cast_shape(
            collider,
            translation,
            rotation,
        ...
        let right_hit = spatial_query.cast_shape(
            collider,
            translation,
            rotation,
        ...
        let forward_hit = spatial_query.cast_shape(
            collider,
            translation,
            rotation,
        ...
        let back_hit = spatial_query.cast_shape(
            collider,
            translation,
            rotation,
        ...
        let is_left_walled = left_hit.is_some_and(|hit| {
            let right = global_transform.right().adjust_precision();
            hit.normal1.angle_between(right) <= 45.0_f32.to_radians()
        });
        let is_right_walled = right_hit.is_some_and(|hit| {
            let left = global_transform.left().adjust_precision();
            hit.normal1.angle_between(left) <= 45.0_f32.to_radians()
        });

        let is_front_walled = forward_hit.is_some_and(|hit| {
            let back = global_transform.back().adjust_precision();
            hit.normal1.angle_between(back) <= 45.0_f32.to_radians()
        });
        let is_back_walled = back_hit.is_some_and(|hit| {
            let front = global_transform.forward().adjust_precision();
            hit.normal1.angle_between(front) <= 45.0_f32.to_radians()
        });

        let mut dir = Vec3::ZERO;
        if is_left_walled {
            dir = left_hit.unwrap().normal1;
        } else if is_right_walled {
            dir = right_hit.unwrap().normal1;
        } else if is_front_walled {
            dir = forward_hit.unwrap().normal1;
        } else if is_back_walled {
            dir = back_hit.unwrap().normal1;
        }
        ...
        if is_walled {
            ...
            commands.entity(entity).remove::<Walled>();
            movement.gravity = movement.max_gravity;
        }
    }
}
...
impl Default for CharacterMovementSettings {
    fn default() -> Self {
        Self {
            ...
            max_gravity: Vector::new(0.0, -9.81 * 2.0, 0.0),
...
fn apply_movement_damping(
    ...
    for (movement, mut linear_velocity, grounded) in &mut query {
        // Approximate exponential decay. We could use `LinearDamping` for this,
        // but we don't want to dampen movement along the Y axis.
        let damping = if grounded.is_some() {
            1.0
        } else {
            continue;
        };
        linear_velocity.x *= 1.0 / (1.0 + delta_secs * movement.damping * damping);
        linear_velocity.z *= 1.0 / (1.0 + delta_secs * movement.damping * damping);
    }
}
